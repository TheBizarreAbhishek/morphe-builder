...
if [ -d morphe-builder ] || [ -f config.toml ]; then
	...
	if git status | grep -q 'is behind\|fatal'; then
		...
		REPO_URL=$(git remote get-url origin 2>/dev/null || echo "https://github.com/AbhishekBabu/morphe-builder")
		git clone "$REPO_URL" morphe-builder --recurse --depth 1
		mv -f config.toml morphe-builder/config.toml
		cd morphe-builder
	fi
else
	pr "Cloning morphe-builder."
	git clone https://github.com/AbhishekBabu/morphe-builder --depth 1
	cd morphe-builder
...
if ask "Open config generator to generate a config?"; then
