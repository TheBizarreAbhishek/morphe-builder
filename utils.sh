		...
		if [ "$tag" = "Patches" ]; then
			...
			if [ "$REMOVE_RV_INTEGRATIONS_CHECKS" = true ]; then
				...
				if ! (
					...
					java -cp "${BIN_DIR}/paccer.jar:${BIN_DIR}/dexlib2.jar" com.bizarre.Main "${file}-zip/extensions/shared.${extensions_ext}" "${file}-zip/extensions/shared-patched.${extensions_ext}" || return 1
...
_req() {
	...
	local curl_args=(-L -c "$TEMP_DIR/cookie.txt" -b "$TEMP_DIR/cookie.txt" --connect-timeout 10 --retry 1 -s -S)
	if [ "$op" != "-" ]; then
		curl_args+=("--fail")
	fi
...
merge_splits() {
	...
	if ! OP=$(java -jar "$APKSIGNER" sign --ks ks-p12.keystore --ks-pass "pass:${KS_PASS}" --key-pass "pass:${KS_PASS}" --ks-key-alias abhishek \
